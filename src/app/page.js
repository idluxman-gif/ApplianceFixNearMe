...
const PT=({p})=>{const m={"$":["Budget","#16a34a"],"$$":["Mid-Range","#EA580C"],"$$$":["Premium","#8B5CF6"]};const[l,c]=m[p]||m["$$"];return <span style={{padding:"2px 8px",borderRadius:5,fontSize:10,fontWeight:600,color:c,background:c+"15"}}>{p} {l}</span>;};
...
{s.website&&<a href={s.website} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",marginTop:8,fontSize:12,color:"#EA580C",textDecoration:"none"}}>🔗 Visit Website</a>}
...
<button onClick={()=>setSf(!sf)} style={{flex:1,padding:"11px",background:"#FFF7ED",color:"#EA580C",border:"none",borderRadius:9,fontSize:13,fontWeight:700,cursor:"pointer"}}>✉️ Get Quote</button>
</div>
...
<select style={{padding:"7px 10px",borderRadius:7,border:"1px solid #CBD5E1",fontSize:12,color:"#64748B"}}><option>Looking for...</option><option>Refrigerator Repair</option><option>Washer Repair</option><option>Dryer Repair</option><option>Dishwasher Repair</option><option>Oven / Range Repair</option><option>Other</option></select>
<button style={{padding:"9px",background:"#EA580C",color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:700,cursor:"pointer"}}>Send</button>
</div></div>}
</div></div></div>);};
...
<div style={{cursor:"pointer"}} onClick={clr}><span style={{fontSize:15,fontWeight:800,color:"#fff"}}>🔧 Appliance<span style={{color:"#EA580C"}}>Fix</span><span style={{color:"#94A3B8",fontWeight:400}}>NearMe</span></span></div>
...
<div style={{display:"inline-block",padding:"4px 12px",background:"rgba(234,88,12,.12)",borderRadius:18,marginBottom:12}}><span style={{fontSize:12,fontWeight:600,color:"#EA580C"}}>🔧 Trusted Appliance Repair Services</span></div>
<h1 style={{margin:"0 0 10px",fontSize:"clamp(24px,5vw,40px)",fontWeight:900,color:"#fff",lineHeight:1.1}}>Find Trusted Appliance<br/><span style={{background:"linear-gradient(135deg,#EA580C,#F59E0B)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Repair Shops</span> Near You</h1>
<p style={{fontSize:14,color:"rgba(255,255,255,.45)",maxWidth:400,margin:"0 auto 20px",lineHeight:1.6}}>{S.length > 0 ? `${S.length}+ verified repair shops across ${AS.length} states.` : "Directory launching soon — verified repair shops across all 50 states."}</p>
...
<input value={q} onChange={e=>{setQ(e.target.value);setPg(1);}} onKeyDown={e=>{if(e.key==="Enter")rr.current?.scrollIntoView({behavior:"smooth"});}} placeholder="City, state, or shop name..." style={{flex:1,border:"none",outline:"none",fontSize:14,padding:"10px 12px",color:"#0F172A",borderRadius:9}}/>
<button onClick={()=>rr.current?.scrollIntoView({behavior:"smooth"})} style={{padding:"10px 18px",background:"#EA580C",color:"#fff",border:"none",borderRadius:9,fontSize:13,fontWeight:700,cursor:"pointer"}}>Search</button>
</div>
{ciSt.length > 0 && <div style={{marginTop:12,display:"flex",flexWrap:"wrap",justifyContent:"center",gap:4}}>
...
{S.length > 0 && <div style={{background:"#fff",borderBottom:"1px solid #E5E7EB",padding:"10px 20px"}}>
<div style={{maxWidth:600,margin:"0 auto",display:"flex",justifyContent:"center",gap:24,flexWrap:"wrap"}}>
{[{n:S.length+"+",l:"Repair Shops"},{n:AS.length,l:"States"},{n:"Verified",l:"Listings"},{n:"4.6",l:"Avg Rating"}].map(x=><div key={x.l} style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:800,color:"#0F172A"}}>{x.n}</div><div style={{fontSize:10,color:"#94A3B8"}}>{x.l}</div></div>)}
</div></div>}
...
<h2 style={{margin:"0 0 12px",fontSize:17,fontWeight:800,color:"#0F172A"}}>{fC?`${fC}, ${fS}`:fS?`Repair Shops in ${fS}`:"All Appliance Repair Shops"}</h2>
{filtered.length===0?<div style={{textAlign:"center",padding:"36px",background:"#fff",borderRadius:12,border:"1px solid #E5E7EB"}}><p style={{color:"#64748B"}}>No repair shops found</p><button onClick={clr} style={{padding:"7px 18px",background:"#0F172A",color:"#fff",border:"none",borderRadius:7,cursor:"pointer",fontWeight:600,fontSize:12}}>Show All</button></div>
...
{paged.length<filtered.length&&<div style={{textAlign:"center",marginTop:16}}><button onClick={()=>setPg(p=>p+1)} style={{padding:"9px 24px",background:"#EA580C",color:"#fff",border:"none",borderRadius:9,fontSize:13,fontWeight:600,cursor:"pointer"}}>Load More ({filtered.length-paged.length} left)</button></div>}</>}
</> : <div style={{textAlign:"center",padding:"48px 20px",background:"#fff",borderRadius:12,border:"1px solid #E5E7EB"}}>
<div style={{fontSize:48,marginBottom:12}}>🔧</div>
<h2 style={{fontSize:20,fontWeight:800,color:"#0F172A",margin:"0 0 8px"}}>Directory Coming Soon</h2>
<p style={{fontSize:14,color:"#64748B",maxWidth:400,margin:"0 auto"}}>We&rsquo;re building the largest directory of verified appliance repair shops in the US. Check back soon!</p>
</div>}
</div>
...
{stSt.map(st=><Link key={st.st} href={getStatePath(st.st)} onClick={(e)=>{e.preventDefault();setFS(st.st);setFC("");setPg(1);rr.current?.scrollIntoView({behavior:"smooth"});}} style={{padding:"9px 10px",background:"#fff",border:"1px solid #E5E7EB",borderRadius:7,cursor:"pointer",textAlign:"left",transition:"all .15s",textDecoration:"none",display:"block"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#EA580C"} onMouseLeave={e=>e.currentTarget.style.borderColor="#E5E7EB"}><div style={{fontSize:12,fontWeight:700,color:"#0F172A"}}>{st.st}</div><div style={{fontSize:10,color:"#64748B"}}>{st.n} shops</div></Link>)}
</div></div></div>}

<div id="about" style={{padding:"36px 20px",maxWidth:640,margin:"0 auto"}}>
<h2 style={{fontSize:18,fontWeight:800,color:"#0F172A",marginBottom:8}}>Why Use ApplianceFixNearMe.com?</h2>
<p style={{fontSize:13,color:"#475569",lineHeight:1.7}}>When your refrigerator stops cooling or your washer won&rsquo;t drain, you need a trusted repair technician fast. ApplianceFixNearMe.com is a free, searchable directory of verified appliance repair shops across the US. Find local professionals, compare ratings, and get your appliances fixed right.</p>
</div>
...
<p style={{fontSize:13,color:"#64748B",margin:0}}>Tips and guides for appliance repair and maintenance</p>
</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:14}}>
{[
{slug:"how-to-find-best-appliance-repair",title:"How to Find the Best Appliance Repair Shop",desc:"Not all repair shops are created equal. Learn what to look for when choosing a technician.",date:"Mar 4, 2026"},
{slug:"when-to-repair-vs-replace-appliances",title:"When to Repair vs. Replace Your Appliances",desc:"Is it worth fixing your old fridge, or is it time for an upgrade? Here's how to decide.",date:"Mar 4, 2026"},
{slug:"common-appliance-problems-diy-fixes",title:"Common Appliance Problems & DIY Fixes",desc:"Some appliance issues are easy to fix yourself. Learn the most common problems and solutions.",date:"Mar 4, 2026"},
...
<div style={{fontSize:10,color:"#EA580C",fontWeight:600,marginBottom:6}}>{a.date}</div>
...
<span style={{display:"inline-block",marginTop:8,fontSize:11,fontWeight:600,color:"#EA580C"}}>Read article →</span>
</a>)}
</div>
...
<div><div style={{fontSize:14,fontWeight:800,color:"#fff",marginBottom:3}}>🔧 Appliance<span style={{color:"#EA580C"}}>Fix</span>NearMe</div><p style={{fontSize:10,maxWidth:220,lineHeight:1.4}}>The largest directory of appliance repair shops in the US.</p></div>
{ciSt.length > 0 && <div><div style={{fontSize:10,fontWeight:600,color:"#fff",marginBottom:4}}>TOP CITIES</div>{ciSt.slice(0,6).map(c=><div key={c.city+c.state} style={{fontSize:10}}>{c.city}, {c.state}</div>)}</div>}
</div>
<div style={{borderTop:"1px solid rgba(255,255,255,.06)",marginTop:12,paddingTop:10,fontSize:9,textAlign:"center"}}>© 2026 ApplianceFixNearMe.com</div>
</footer>
